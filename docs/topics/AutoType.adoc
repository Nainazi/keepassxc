...
In 奈娜子密码本 the default global shortcut for a new configuration is kbd:[Ctrl+Shift+A]. It is only registered when you trigger it. Auto-Type does not read the keyboard in the background. If that database is locked, the unlock dialog explains that Auto-Type is waiting. If no entry matches the target window, a message says so instead of failing silently.

Entry and group editors include sequence templates:

* `{USERNAME}{TAB}{PASSWORD}{ENTER}`
* `{USERNAME}{TAB}{PASSWORD}{TAB}{ENTER}`
* `{PASSWORD}{ENTER}`

Choosing a template fills the custom sequence field. You can still type any other sequence.
