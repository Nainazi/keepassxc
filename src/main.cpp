...
int main(int argc, char** argv)
{
    ...
    if (app.isAlreadyRunning()) {
        if (parser.isSet(lockOption)) {
            ...
            qWarning() << QObject::tr("Another instance of 奈娜子密码本 is already running.").toUtf8().constData();
        }
        return EXIT_SUCCESS;
    }

    if (parser.isSet(lockOption)) {
        qWarning() << QObject::tr("奈娜子密码本 is not running. No open database to lock").toUtf8().constData();
    ...
    if (!Crypto::init()) {
        ...
        MessageBox::critical(nullptr, QObject::tr("奈娜子密码本 - Error"), error);
        return EXIT_FAILURE;
    }
